namespace coronavirus_dashboard_summary.Models
...
module BaseModel = 
    ...
    type MSOAData(redis, date, payload) =
        ...
        override this.query = 
            ...
            SELECT (metric || UPPER(LEFT(key, 1)) || RIGHT(key, -1)) AS metric,
                   ...
                   date,
                   (
                       CASE WHEN value::TEXT <> 'null' THEN TRIM(BOTH '\"' FROM value::TEXT)
                       ELSE '-999999'
                       END
                   ) AS value,
                   RANK() OVER ( PARTITION BY (metric) ORDER BY date DESC ) AS rank
            " 
            + $"FROM covid19.time_series_p{this.date.partitionDate}_{this.partition} AS ts "
    ...
    type GeneralData(redis, date, payload) =
        ...
        override this.query =
                ...
                "
                + $"FROM covid19.time_series_p{date.partitionDate}_{this.partition} AS ts "
                + "    JOIN covid19.release_reference AS rr ON rr.id = release_id
                       JOIN covid19.metric_reference  AS mr ON mr.id = metric_id
                       JOIN covid19.area_reference    AS ref ON ref.id = area_id
